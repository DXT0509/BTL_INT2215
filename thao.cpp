...
double tinhgoc(double x1, double y1, double x2, double y2) {
    ...
    double* angle_rad = new double(atan2(*dy, *dx));
    double* angle_deg = new double(*angle_rad * 180.0 / M_PI);
...
int main(int argc, char* argv[]) {
    ...
    SDL_Surface* image11 = IMG_Load("long.png");
    ...
    longchim = SDL_CreateTextureFromSurface(renderer, image11);
    ...
    while ( quit == false )
    {
        ...
        while ( SDL_PollEvent(&event) != 0 )
        {
            ...
            {
                if ( live > 0 && finishround[5] == false && clickplay == true  && live > 0) clickexit = true;
            ...
            else if ( event.type == SDL_MOUSEBUTTONDOWN)
            {
                    ...
                    if ( live > 0 ) {
                        ...
                        if ( danthunder == false && dead <= 300 ) {
                            ...
                            else if ( sodan == 4 )
                            {
                                ...
                                bullet.push_back({x+sizetaux/2-8,y-sizetauy-5});
                            }
                        }
                        else if ( blaster.f == 0 ) {
                            ...
                            else if ( x1.s.s != 0 )
                            {
                                ...
                                int r1 = rand()%2;
                                if ( r1 % 2 == 1 ) vtlong.push_back({info[x1.f].s.f+sizegax/2,info[x1.f].s.s+sizegay});
                            }
                            blaster.f = 12;
                ...
                if ( motionremuse1 == true && clickabout == false && clickplay == false)
                {
                    clickplay = true ;
                    inp >> man >> sodan >> loaidan >> levelthunder >> live >> score;
                    if ( loaidan == 3 )
                    {
                        danthunder = true ;
                        loaidan = 1;
            ...
            {
                if (event.key.keysym.sym == SDLK_ESCAPE && clickplay == true && timeendgame == 0 && live > 0 ) {
                    clickesc = true;
...
{
    timestop();
    SDL_Rect bigwarning = SDL_Rect{chieudai/2-823/2,300,823,124};
...
bool ktgiaonhau1(SDL_Rect rect1, double angle1, SDL_Rect rect2) {
    double cx1 = (rect1.x + rect1.w / 2.0);
    double cy1 = (rect1.y + rect1.h / 2.0);
    double cx2 = (rect2.x + rect2.w / 2.0);
    double cy2 = (rect2.y + rect2.h / 2.0);
    double angleRad = angle1 * M_PI / 180.0;
    double halfWidth1 = (rect1.w / 2.0);
    double halfHeight1 = (rect1.h / 2.0);
    double halfWidth2 = (rect2.w / 2.0);
    double halfHeight2 = rect2.h / 2.0;
    double cosA = cos(angleRad);
    double sinA = sin(angleRad);
    double x1 = cx1 + halfWidth1 * cosA - halfHeight1 * sinA;
    double y1 = cy1 + halfWidth1 * sinA + halfHeight1 * cosA;
    double x2 = cx1 - halfWidth1 * cosA - halfHeight1 * sinA;
    double y2 = cy1 - halfWidth1 * sinA + halfHeight1 * cosA;
    double x3 = cx1 - halfWidth1 * cosA + halfHeight1 * sinA;
    double y3 = cy1 - halfWidth1 * sinA - halfHeight1 * cosA;
    double x4 = cx1 + halfWidth1 * cosA + halfHeight1 * sinA;
    double y4 = cy1 + halfWidth1 * sinA - halfHeight1 * cosA;
    ...
    cx2 = cx1Temp;
    cy2 = cy1Temp;
    double halfWidth1Temp = halfWidth1;
    double halfHeight1Temp = halfHeight1;
    halfWidth1 = halfWidth2;
    halfHeight1 = halfHeight2;
    halfWidth2 = halfWidth1Temp;
    halfHeight2 = halfHeight1Temp;
    cosA = cos(-angleRad);
    sinA = sin(-angleRad);
    x1 = cx1 + halfWidth1 * cosA - halfHeight1  *sinA;
    y1 = cy1 + halfWidth1 * sinA + halfHeight1  *cosA;
    x2 = cx1 - halfWidth1 * cosA - halfHeight1 *sinA;
    y2 = cy1 - halfWidth1 *sinA + halfHeight1 * cosA;
    x3 = cx1 - halfWidth1 * cosA + halfHeight1 *sinA;
    y3 = cy1 - halfWidth1 * sinA - halfHeight1 * cosA;
    x4 = cx1 + halfWidth1 * cosA + halfHeight1 * sinA;
    y4 = cy1 + halfWidth1 * sinA - halfHeight1 * cosA;
...
void timestop()
{
    ...
    for ( auto &u : vtlong)
    {
        SDL_Rect* vt = new SDL_Rect{u.f,u.s,30,10};
        SDL_RenderCopy(renderer, longchim, NULL, vt);
        delete vt;
    }
    for ( auto &u : vtegg)
    {
...
void instart()
{
    ...
    if ( motionstart == false ) SDL_RenderCopy(renderer,play1,NULL,&bigplay1);
    if ( motionstart == true ) SDL_RenderCopy(renderer,play2,NULL,&bigplay1);
    SDL_Rect bigabout1 = {chieudai/2-174/2,750,174,74};
    if ( motionabout == false )SDL_RenderCopy(renderer,about1,NULL,&bigabout1);
    else SDL_RenderCopy(renderer,about2,NULL,&bigabout1);
}
...
void inman()
{
    ...
    int w=200;
    SDL_Surface* over;
    if ( finishround[1] == false ) over = TTF_RenderText_Solid(font, "WAVE 1", textcolor);
    else if ( finishround[2] == false ) over = TTF_RenderText_Solid(font, "WAVE 2", textcolor);
    else if ( finishround[3] == false ) over = TTF_RenderText_Solid(font, "WAVE 3", textcolor);
    else if ( finishround[4] == false ) over = TTF_RenderText_Solid(font, "WAVE 4", textcolor);
    else {
        if ( timeman == 200 ) {
            Mix_PlayMusic(musicboss, -1);
            Mix_VolumeMusic(50);
        }
        over = TTF_RenderText_Solid(font, "BOSS IS COMING !!!", textcolor);
        w=300;
    }
    ...
    SDL_Rect* vt = new SDL_Rect{chieudai/2-200/2,chieurong/2-100/2,w,100};
...
void gameover()
{
    SDL_Surface* over = TTF_RenderText_Solid(font, "GAME OVER", textcolor);
    SDL_Texture* gover = SDL_CreateTextureFromSurface(renderer, over);
    int* x = new int(300);
    int* y = new int(300);
    score += score1;
    SDL_QueryTexture(gover, NULL, NULL, x, y);
    SDL_Rect* vt = new SDL_Rect{chieudai / 2 - 200 / 2, chieurong / 2 - 150, 200, 75};
    SDL_RenderCopy(renderer, gover, NULL, vt);
    ...
    *s = "LAST SCORE: " + *s;
    ...
    over = TTF_RenderText_Solid(font, s->c_str(), textcolor);
    gover = SDL_CreateTextureFromSurface(renderer, over);
    SDL_QueryTexture(gover, NULL, NULL, x, y);
    *vt = {chieudai / 2 - 310 / 2, chieurong / 2 - 100, 310, 75};
    SDL_RenderCopy(renderer, gover, NULL, vt);
...
void inlive()
{
    SDL_Surface* over = nullptr;
    SDL_Texture* gover = nullptr;
    int* x = new int(300);
    int* y = new int(300);

    if (live == 3)
        over = TTF_RenderText_Solid(font, "LIVE: 3", textcolor);
    else if (live == 2)
        over = TTF_RenderText_Solid(font, "LIVE: 2", textcolor);
    else if (live == 1)
        over = TTF_RenderText_Solid(font, "LIVE: 1", textcolor);
    else
        over = TTF_RenderText_Solid(font, "LIVE: 0", textcolor);

    if (over) {
        gover = SDL_CreateTextureFromSurface(renderer, over);
        if (gover) {
            SDL_QueryTexture(gover, NULL, NULL, x, y);
            SDL_Rect vt = {20, -10, 100, 50};
            SDL_RenderCopy(renderer, gover, NULL, &vt);
            SDL_DestroyTexture(gover);
        }
        SDL_FreeSurface(over);
...
void inscore()
{
    ...
    *s = "SCORE: " + *s;
    ...
    SDL_Surface* over = TTF_RenderText_Solid(font, s->c_str(), textcolor);
    SDL_Texture* gover = SDL_CreateTextureFromSurface(renderer, over);

    int* yValue = new int(300);
    SDL_Rect* vt = new SDL_Rect{20, 20, 180, 50};
    int* xValue = new int;
...
void xulygachet( int i)
{
    ...
    Mix_PlayChannel(-1, chickensound, 0);
    score1+=100;
    int r = rand()%50+1;
    if ( info.size() == 1 ) {
        if ( sodan == 1  ) r=1;
    ...
    for ( auto &u : info)
    {
        ...
        for ( auto &u1 : bullet)
        {
            if ( u1.f+sizedanx >= u.s.f && u1.f <= u.s.f+sizegax && u1.s <= u.s.s+sizegay && u1.s >= u.s.s)
            {
                bullet.erase(bullet.begin()+j);
                int r = rand()%2;
                if ( r == 1 ) vtlong.push_back({u.s.f+sizegax/2,u.s.s+sizegay});
                if ( loaidan == 1 ) u.f-=3;
                else u.f-=4;
                if ( u.f <= 0 )
                {
...
void inplay(int x, int y)
 {
    ...
    if ( timeendgame <= 800 ) {solvegame();
    inlive();
    inscore();
    }
    if ( live <= 0 ) return ;
    if ( timeendgame <= 800 ) xulidan();
}
